...
let get_leaves network ~person =
  Set.fold network ~init:[] ~f:(fun acc (p1, p2) ->
    if String.equal person p1 then acc @ [ p1, p2 ] else acc)
;;

let rec rec_find_friend_group network ~q ~explored : Person.t list =
  match q with
  | [] -> explored
  | head :: tail ->
    let new_explored =
      List.fold
        (get_leaves network ~person:head)
        ~init:[]
        ~f:(fun acc (_, p2) ->
        if not (List.mem explored p2 ~equal:String.equal)
        then acc @ [ p2 ]
        else acc)
    in
    let new_q = tail @ new_explored in
    rec_find_friend_group network ~q:new_q ~explored:(explored @ new_explored)
;;

let find_friend_group network ~person : Person.t list =
  let q = [ person ] in
  let explored = [ person ] in
  rec_find_friend_group network ~q ~explored
;;
